function [out,motionInfo] = kgbvs(img, kernelFunFeats, kernelFunMap, param, prevMotionInfo)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                                                     %                            
% This computes the KGBVS map for an image and puts it in master_map.                                  %
%                                                                                                     %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
defaultKernelFunFeats = @(x,p)(exp( -1 * x / (2 * p^2)));
defaultKernelFunMap = @(x,p)(exp( -1 * x / (2 * p^2)));
if nargin<2
    kernelFunFeats = defaultKernelFunFeats;
    kernelFunMap = defaultKernelFunMap;
end
...
if ( (nargin < 5) || (~exist('prevMotionInfo')) )
    prevMotionInfo = [];
end

if ( param.useIttiKochInsteadOfGBVS )
    mymessage(param,'NOTE: Computing STANDARD Itti/Koch instead of Kernelized Graph-Based Visual Saliency (KGBVS)\n\n');
end
    ...
    for typei = 1 : numtypes
        if ( param.activationType == 1 )
            for lev = param.levels                
                ...
                i = i + 1;
                [allmaps{i}.map,tmp] = NM_graphsalapply( mapsobj.maps.val{typei}{lev} , ...
                    kernelFunFeats, param.sigma_frac_act, ...
                    grframe, 1 , 2 , param.tol );
...
for i=1:length(allmaps)
    ...
    if ( param.normalizationType == 1 )
        ...
        algtype = 4;
        [norm_maps{i}.map,tmp] = NM_graphsalapply( allmaps{i}.map , kernelFunMap, param.sigma_frac_norm, grframe, param.num_norm_iters, algtype , param.tol );        
...
for fmapi = 1 : length(mapnames)
  if ( param.normalizeTopChannelMaps) 
      ...
      if ( param.normalizationType == 1 )
          algtype = 4;
          [cmaps{fmapi},tmp] = NM_graphsalapply( cmaps{fmapi}, kernelFunMap, param.sigma_frac_norm, grframe, param.num_norm_iters, algtype , param.tol );
      elseif ( param.normalizationType == 2 )
          algtype = 1;
          [cmaps{fmapi},tmp] = NM_graphsalapply( cmaps{fmapi}, kernelFunMap, param.sigma_frac_norm, grframe, param.num_norm_iters, algtype , param.tol );
      else
