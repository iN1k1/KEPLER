...
function [Anorm,iters] = NM_graphsalapply( A, kernelFun, kernelFreePar, frame, num_iters , algtype , tol )
...
if ~isempty(kernelFreePar)
    kernelFreePar = kernelFreePar * mean(size(A));
end
%Dw = exp( -1 * frame.D / (2 * sig^2) );
Dw = kernelFun(frame.D, kernelFreePar);
