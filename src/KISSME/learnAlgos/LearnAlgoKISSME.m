...
classdef LearnAlgoKISSME < LearnAlgo
    ...
    methods
        ...
        function s = learnPairwise(obj,X,idxa,idxb,matches)
            ...
            covMatches    = SOPD(double(X),idxa(matches),idxb(matches)) / sum(matches);
            %A = X(:, idxa(matches))';
            %B = X(:, idxb(matches))';
            %AB = A-B;
            %covMatches = (AB'*AB) / sum(matches);

            % Eqn. (13) - sum of outer products of pairwise differences (dissimilar pairs)
            % normalized by the number of dissimilar pairs.
            covNonMatches = SOPD(double(X),idxa(~matches),idxb(~matches)) / sum(~matches);
            %A = X(:, idxa(~matches))';
            %B = X(:, idxb(~matches))';
            %AB = A-B;
            %covNonMatches = (AB'*AB) / sum(~matches);
            t = toc;
            ...
            regp = 0.0;
            regn = 0.0;
            s.M = inv(covMatches + regp*eye(size(covMatches))) - obj.p.lambda * inv(covNonMatches + regn*eye(size(covNonMatches)));   
            if obj.p.pmetric
